...
; run all the tests by applying the solution to the problem
(defmacro define-test [test-group]
  (let [[number pair] test-group
        [prob soln] pair]
    `(deftest ~(symbol (str "test-" number)) '~(prob soln))))

(macroexpand-1 '(define-test (first by-num)))
; (first by-num)
; (define-test (first by-num))
; (define-test ["33" [prob49 sol49]])
; (map #(define-test %) by-num)
; (class (first by-num))
...
;       :let [[prob soln] pair]]
;   `(deftest ~(symbol number) (testing ~number (~prob ~soln))))
